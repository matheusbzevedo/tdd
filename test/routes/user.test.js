...
test('Deve inserir um usuário com sucesso', () => {
    return request(app).post('/users').send({ name: 'Walter White', email, senha: '123456'}).then(response => {
...
test('Deve não deixar inserir um usuário sem nome', () => {
    return request(app).post('/users').send({ email: 'walter@mail.com', senha: '123456' }).then(response => {
...
test('Deve não deixar inserir um usuário sem email', async () => {
    const result = await request(app).post('/users').send({ name: 'Walter White', senha: '123456' });
    expect(result.status).toBe(400);
    expect(result.body.error).toBe('Email é um atributo obrigatório');
});

test('Deve não deixar inserir um usuário sem senha ', done => {
    request(app).post('/users').send({ name: 'Walter White', email: 'walterwhite@mail.com' }).then(response => {
        expect(response.status).toBe(400);
